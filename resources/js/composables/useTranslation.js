export default function useTranslation() {
  ...
  const __ = (key, replacements = {}) => {
    ...
    // console.log({ about: 'translations mixin', translation, key, replacements })
    if (translation == '') {
      return key
    }
    return makeReplacements(translation, replacements)
  ...
  const trans_choice = (key, number, replacements = {}) => {

    let translation = window._translations[key] || key || '';
    // const message = (key, replacements)
    const message = makeReplacements(translation, replacements)

    replacements.count = number?.toString() || 1

    return makeReplacements(choose(message, number, 'fr'), replacements)
  ...
  const makeReplacements = (message, replacements) => {
   ...
   */
  const choose = (message = '', number, lang = 'fr') => {
  ...
  const getPluralIndex = (lang, number) => {
    switch (lang) {
      case 'az':
      case 'az_AZ':
      case 'bo':
